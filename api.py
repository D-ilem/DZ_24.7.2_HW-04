...
class PetFriends:
    ...
    '''Вариант без MultipartEncoder и без Content-Type'''
    ...
    def get_pets(self, filter):
        pass

    def add_new_pet_without_photo(self, auth_key: json, name: str, animal_type: str,
                                  age: str) -> json:
        """Метод отправляет на сервер данные о добавляемом питомце без фотографии возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""
    ...
    def add_photo(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод позволяет добавить фото питомцу (по id), возвращает СТАТУС запроса и ответ
        в формате JSON с указанием информации о фото"""
